...
protected:
	void EnsureEncoder()
	{
		if (encoder != NULL)
			return;
		encoder = celt_encoder_create(samplingRate, 1, NULL);
	}
	void EnsureDecoder()
	{
		if (decoder != NULL)
...
public:
	CeltEncoder(celt_int32 rate, int frame_size) : encoder(NULL), decoder(NULL), frameSize(frame_size), samplingRate(rate)
	{
	...
	static NAN_METHOD(SetBitrate)
	{
		...
		celt_encoder_ctl(self->encoder, CELT_SET_BITRATE(bitrate));
	}
